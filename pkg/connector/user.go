...
func (o *userResourceType) List(
	...
	if o.enterpriseID != "" {
		...
		if err != nil {
			return nil, "", nil, fmt.Errorf("slack-connector: pagination bag error in user list: %w", err)
		}
		...
		if err != nil {
			return nil, "", nil, fmt.Errorf("slack-connector: pagination bag NextToken error in user list: %w", err)
		}
	}
...
func (o *userResourceType) CreateAccount(
	...
	if err != nil {
		return nil, nil, nil, fmt.Errorf("slack-connector: failed to create workspace resource ID for new user: %w", err)
	}

	r, err := userResource(ctx, user, parentResourceID)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("slack-connector: failed to build user resource for newly created account: %w", err)
	}
