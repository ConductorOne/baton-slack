...
func (o *workspaceResourceType) List(
	...
	if err != nil {
		return nil, "", nil, fmt.Errorf("slack-connector: pagination bag error in workspace list: %w", err)
	}
	...
	if err != nil {
		return nil, "", nil, fmt.Errorf("slack-connector: pagination bag NextToken error in workspace list: %w", err)
	}
	...
	if err != nil {
		return nil, "", nil, fmt.Errorf("slack-connector: failed to build workspace resource list: %w", err)
	}
...
func (o *workspaceResourceType) Grants(
	...
	if err != nil {
		return nil, "", nil, fmt.Errorf("slack-connector: pagination bag error in workspace grants: %w", err)
	}
	...
	pageToken, err := bag.NextToken(nextCursor)
	if err != nil {
		return nil, "", nil, fmt.Errorf("slack-connector: pagination bag NextToken error in workspace grants: %w", err)
	}
	...
	for _, user := range users {
		...
		if err != nil {
			return nil, "", nil, fmt.Errorf("slack-connector: failed to create user resource ID for workspace grant: %w", err)
		}

		if user.IsPrimaryOwner {
			rr, err := roleResource(ctx, PrimaryOwnerRoleID, resource.Id)
			if err != nil {
				return nil, "", nil, fmt.Errorf("slack-connector: failed to create primary owner role resource: %w", err)
			}
		...
		if user.IsOwner {
			...
			if err != nil {
				return nil, "", nil, fmt.Errorf("slack-connector: failed to create owner role resource: %w", err)
			}
			rv = append(rv, grant.NewGrant(rr, RoleAssignmentEntitlement, userID))
		}
		...
		if user.IsAdmin {
			...
			if err != nil {
				return nil, "", nil, fmt.Errorf("slack-connector: failed to create admin role resource: %w", err)
			}
			rv = append(rv, grant.NewGrant(rr, RoleAssignmentEntitlement, userID))
		}
		...
		if user.IsRestricted {
			if user.IsUltraRestricted {
				...
				if err != nil {
					return nil, "", nil, fmt.Errorf("slack-connector: failed to create single channel guest role resource: %w", err)
				}
				rv = append(rv, grant.NewGrant(rr, RoleAssignmentEntitlement, userID))
			} else {
				rr, err := roleResource(ctx, MultiChannelGuestRoleID, resource.Id)
				if err != nil {
					return nil, "", nil, fmt.Errorf("slack-connector: failed to create multi channel guest role resource: %w", err)
				}
				rv = append(rv, grant.NewGrant(rr, RoleAssignmentEntitlement, userID))
			}
		}
		...
		if user.IsInvitedUser {
			...
			if err != nil {
				return nil, "", nil, fmt.Errorf("slack-connector: failed to create invited member role resource: %w", err)
			}
			rv = append(rv, grant.NewGrant(rr, RoleAssignmentEntitlement, userID))
		}
		...
		if !user.IsRestricted && !user.IsUltraRestricted && !user.IsInvitedUser && !user.IsBot && !user.Deleted {
			...
			if err != nil {
				return nil, "", nil, fmt.Errorf("slack-connector: failed to create member role resource: %w", err)
			}
			rv = append(rv, grant.NewGrant(rr, RoleAssignmentEntitlement, userID))
		}
		...
		if user.IsBot {
			...
			if err != nil {
				return nil, "", nil, fmt.Errorf("slack-connector: failed to create bot role resource: %w", err)
			}
			rv = append(rv, grant.NewGrant(rr, RoleAssignmentEntitlement, userID))
		}
		...
		if o.enterpriseID != "" {
			if user.Enterprise.IsPrimaryOwner {
				...
				if err != nil {
					return nil, "", nil, fmt.Errorf("slack-connector: failed to create organization primary owner role resource: %w", err)
				}
				rv = append(rv, grant.NewGrant(rr, RoleAssignmentEntitlement, userID))
			}
			if user.Enterprise.IsOwner {
				...
				if err != nil {
					return nil, "", nil, fmt.Errorf("slack-connector: failed to create organization owner role resource: %w", err)
				}
				rv = append(rv, grant.NewGrant(rr, RoleAssignmentEntitlement, userID))
			}
			if user.Enterprise.IsAdmin {
				...
				if err != nil {
					return nil, "", nil, fmt.Errorf("slack-connector: failed to create organization admin role resource: %w", err)
				}
				rv = append(rv, grant.NewGrant(rr, RoleAssignmentEntitlement, userID))
			}
		}
