...
func TestWorkspaceGrantAndRevoke(t *testing.T) {
	...
	t.Run("Grant operation for workspace with invalid principal", func(t *testing.T) {
		...
		// Verify the error.
		require.Error(t, err)
		assert.Contains(t, err.Error(), "slack-connector: only users can be assigned to a workspace")
	})
	...
	t.Run("Revoke operation for workspace with invalid principal", func(t *testing.T) {
		...
		// Verify the error.
		require.Error(t, err)
		assert.Contains(t, err.Error(), "slack-connector: only users can be revoked from a workspace")
	})
